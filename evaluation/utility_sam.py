...
class SAMLine:
	...
	def CalcReadLengthFromCigar(self, switch_ins_and_dels=False):
        ...
        #cal aligned bases
        def CalcAlignedBaseFromCigar(self, switch_ins_and_dels=False):
		split_cigar = self.SplitCigar();
		alignment_length = 0;
		i = 0;
		while i < len(split_cigar):
			...
			#     Sum of lengths of the M/I/S/=/X operations shall equal the length of SEQ.
			# if (cigar_op in 'MISH=X'):
			if ((switch_ins_and_dels == False and (cigar_op in 'M=X')) or
				(switch_ins_and_dels == True and (cigar_op in 'MDSH=X'))):
				alignment_length += cigar_count;
			i += 1;
		return alignment_length;
...
def getExtendedCIGAR(string1, string2, countMutations = True):
        #print "len1 = %d, len2 = %d" %(len(string1), len(string2))
