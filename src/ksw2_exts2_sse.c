...
#define __dp_code_block2 \
	...
	if (with_cigar) { // backtrack
		...
		if (!ez->zdropped && !(flag&KSW_EZ_EXTZ_ONLY))
			ksw_backtrack_R(km, 1, rev_cigar, long_thres, (uint8_t*)p, off, off_end, n_col_*16, tlen-1, qlen-1, &ez->m_cigar, &ez->n_cigar, &ez->cigar, q2, flag, ez);
		else if (ez->max_t >= 0 && ez->max_q >= 0)
			ksw_backtrack_R(km, 1, rev_cigar, long_thres, (uint8_t*)p, off, off_end, n_col_*16, ez->max_t, ez->max_q, &ez->m_cigar, &ez->n_cigar, &ez->cigar, q2, flag, ez);
