...
void ksw_exts2_sse(void *km, int qlen, const uint8_t *query, int tlen, const uint8_t *target, int8_t m, const int8_t *mat,
				   int8_t gapo, int8_t gape, int8_t gapo2, int8_t noncan, int zdrop, int flag, ksw_extz_t *ez, uint8_t *junc);
//frag
...
static inline void ksw_backtrack_D(void *km, int is_rot, int is_rev, int min_intron_len, const uint8_t *p, const int *off, const int *off_end, int n_col, int i0, int j0,
...
static inline void ksw_backtrack_R(void *km, int is_rot, int is_rev, int min_intron_len, const uint8_t *p, const int *off, const int *off_end, int n_col, int i0, int j0,
								 int *m_cigar_, int *n_cigar_, uint32_t **cigar_, int jumpdp, int flag, ksw_extz_t *ez)
{ // p[] - lower 3 bits: which type gets the max; bit
    // fprintf(stderr, "before change socre = %d\n", ez->score);
	int n_cigar = 0, m_cigar = *m_cigar_, i = i0, j = j0, r, state = 0;
	int N_len = 0;
	int total_len = 1;
	uint32_t *cigar = *cigar_, tmp;
	while (i >= 0 && j >= 0) { // at the beginning of the loop, _state_ tells us which state to check
		...
		if (state == 0)
		{
			cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, 0, 1); // match
			--i;
			--j;
			total_len++;
		}
		else if (state == 1 || (state == 3 && min_intron_len <= 0)) // deletion
		{
		...
		{
			cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, 3, 1);// intron
			--i;
			if (N_len < total_len)
			{
		...
		{
			cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, 1, 1);// insertion
			--j;
			total_len++;
		}
	}
    //first deletion or intron
	if (i >= 0) cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar,  min_intron_len > 0 && i >= min_intron_len? 3:2, i + 1);
    //if (i >= 0)
    //{
    //    if (min_intron_len > 0 && i > min_intron_len)
    //    {
    //        cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar,  3, i + 1);
    //        if (flag&KSW_EZ_EXTZ_ONLY)
    //            ez->max += jumpdp;
    //        else
    //            ez->score += jumpdp;
    ...
    //    else
    //    {
    //        cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar,  2, i + 1);
    //    }
    //}
	if (j >= 0) cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, 1, j + 1); // first insertion
	if (!is_rev)
